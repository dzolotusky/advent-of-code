...
while line_num < len(content):
    ...
    pre_length = len(stacks[move_from])
    stacks[move_to] += stacks[move_from][-num_to_move:]
    stacks[move_from] = stacks[move_from][:-num_to_move]
    post_length = len(stacks[move_from])
