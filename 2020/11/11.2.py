            ...
            for j_delta in range(-1, 2):
                ...
                if -1 < j + j_delta < len(test_grid[i]):
                    if test_grid[i+i_delta][j+j_delta] == "#":
                        ...
                        if test_grid[i + i_delta][j + j_delta] == ".":
                            steps = 2
                            while(-1 < i + (steps * i_delta) < len(test_grid) and
                                  -1 < j + (steps * j_delta) < len(test_grid[i])):
                                if test_grid[i + steps * i_delta][j + steps * j_delta] == "#":
                                    num_occ += 1
                                if test_grid[i + steps * i_delta][j + steps * j_delta] != ".":
                                    break
                                steps += 1
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if grid[i][j] == "L" and count_adjacent(grid, i, j) == 0:
                ...
                if grid[i][j] == "#" and count_adjacent(grid, i, j) > 4:
                    new_grid[i][j] = "L"
                    made_change = True
                else:
