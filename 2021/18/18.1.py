...
add_tree = expression_trees[0]
for tree_index in range(1, len(expression_trees)):
    add_tree = Node(None, add_tree, expression_trees[tree_index])
    counter = 0
    while explode(add_tree) or split(add_tree):
        #print_tree(add_tree)
        counter += 1

part1 = calculate_magnitude(add_tree)
